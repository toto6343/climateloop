...
> 아래 `[리포지토리 URL 기입]` 은 공개 준비 중입니다. 지금은 내려받은 소스에서
> 두 번째 줄부터 진행하세요.

```bash
git clone [리포지토리 URL 기입] && cd climateloop
...
**기준일: 2026-08-21.** 이 표가 실제 동작 범위에 대한 단일 기준입니다.
...
| 기능 | 현재 상태 |
|---|---|
| **기상 데이터 카드** (연평균 일사량·풍속·강수량·기온) | 미구현. `weather_data` 테이블은 정의만 되어 있고 데이터가 없습니다 |
| 두 지역 나란히 비교 | 부분 구현. 지도 마커 크기와 드롭다운으로 지역을 바꿔가며 비교할 수 있지만, 두 지역의 점수·배출량을 한 화면에 나란히 놓는 비교 뷰는 없습니다 |
...
| `POST /calculate` | ❌ (면책 필드 없음 — 화면 각주로 대체) | ✅ (`data_source` / `data_source_detail.covered_sources`) |
| `POST /regions` | — (배출계수 미사용) | ✅ (`unit` = "MWh (추정)" / `note` / `scale` / `data_source`) |
| `GET /api/weather/scenario` | — | ✅ (`source` = `"live"`/`"fallback"`) |
...
| 기능 | 설명 | 연동 상태 |
|---|---|---|
| 지역 선택 | 드롭다운으로 시·도를 고르면 지도 강조 + 발전원 구성 패널이 갱신됩니다 | ✅ 내장 좌표 |
...
| 결과 저장 | PDF 리포트 다운로드 | ✅ 브라우저에서 생성 |
| 기상 데이터 카드 | 연평균 일사량·풍속·강수량·기온 표시 | ❌ 미구현 |
...
```mermaid
flowchart TD
    A[지역 선택] --> B[기상 시나리오 선택]
    B --> C[발전원별 적합도 확인]
    C --> D[에너지 믹스 슬라이더 조작]
    D --> E[탄소 배출 · 전력망 · 점수 갱신]
    E --> F{목표 점수에 닿았나?}
    F -- 아니오 --> G[다음 행동 추천 확인 · 원클릭 적용]
    G --> D
    F -- 예 --> H[AI 어시스턴트에 이유 질문]
    H --> I[PDF 리포트 저장]
```

위 순서는 권장 흐름이지 강제 단계가 아닙니다. 슬라이더를 움직이면 적합도·배출량·점수·해설이
한 번에 갱신되므로 어느 단계에서든 되돌아갈 수 있습니다.
...
|---|---|---|---|---|
| 기상 관측 | 지상관측자료 (기온·강수·풍속·일사량) | 기상청 API허브 | 시간/일 단위, 지점별 | 기상 데이터 카드, 물리 기반 발전량 추정([9.5](#95-향후-확장-물리-기반-추정)) |
...
| 용도 | 창구 |
|---|---|
| 버그 신고 · 기능 제안 | [리포지토리 URL 기입] 의 Issues |
| 질문 · 활용 사례 공유 | [문의 이메일 또는 Discussions 링크 기입] |
...
| 프론트엔드 | Next.js 16.2.9 · React 19.2.4 · Tailwind CSS 4 · TypeScript 5 | |
...
| 출처 | 사유 |
|---|---|
| 기상청 단기예보(초단기실황) + 기상특보 | **기상특보 미승인.** 두 호출이 모두 성공해야 시나리오를 추천하므로 현재 배지가 표시되지 않습니다 |
